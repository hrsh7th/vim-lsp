    ...
    if !empty(a:data['response']['result']) && !empty(a:data['response']['result']['signatures'])
        ...
        if exists('l:parameter')
            ...
            if !empty(l:parameter_doc)
                call add(l:contents, '')
                call add(l:contents, s:trim_documentation(l:parameter_doc))
                call add(l:contents, '')
            endif
        endif

        if has_key(l:signature, 'documentation')
            call add(l:contents, s:trim_documentation(l:signature['documentation']))
        endif
...
function! s:trim_documentation(doc) abort
    if type(a:doc) == type({})
        let a:doc['value'] = join(split(a:doc['value'], "\n")[0 : 5], "\n")
        return a:doc
    endif
    return join(split(a:doc, "\n")[0 : 5], "\n")
endfunction
